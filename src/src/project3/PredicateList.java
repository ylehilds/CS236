    ...
    extends List
{
     ...
     *   </a>
     */
    protected boolean createAllCombinations(int i) throws ParserException
{
    boolean test = false;
	boolean returnsTrueOrFalse = false;
	Constant c = null;
	boolean assurance = false;
	Iterator <Constant> iter = Domain.iterator(); 
	if (variables.length == i)    // BASE CASE
		{
			return  checkToSeeIfTrue();
		}
	while (iter.hasNext())
	{
	c = iter.next();
	setVariableToValue(variables[i], c);
	if (i< variables.length-1)
	{
	if (!allConstants() || keepOnGoing(checkToSeeIfTrue()))
	{	
		if (!returnsTrueOrFalse){
			returnsTrueOrFalse = createAllCombinations(i+1);}
		else{
			createAllCombinations(i+1);}
	}
	else{
		returnsTrueOrFalse = true;}
		
		//else return true;	
	}
	else { 
		if (variables.length !=0 && checkToSeeIfTrue())
		{
			returnsTrueOrFalse = true;
		}
	}
	}
return returnsTrueOrFalse;
}
    ...
    protected boolean checkToSeeIfTrue() throws ParserException
    {
...
//    	Predicate p = (Predicate) iter.next();
    	...
    	{
 /*  If p could be proven true by the facts or the rules then call the method saveResult().
  *   In the PredicateList class this method does nothing, but in the Query class this method is overridden 
  *   and saves the predicate so it can be printed later.
*/   		
	if ((Project3.datalogProgram.getFactList().canProve((Predicate)nodes.get(i)) || Project3.datalogProgram.getRuleList().canProve((Predicate)nodes.get(i))) == false)
	{
		temp = false;
	}
	else
...
//    	}
	//if (Project3.datalogProgram.getRuleList().canProve(p) == true)
	//{saveResult();
	//return true;}
